...
protected:
    ...
    struct common_node {
        ...
        explicit common_node(typename associative_container<tkey, tvalue>::key_value_pair *keys_and_values,
            common_node **subtrees,
            size_t t);
...
protected:
    ...
    inline void swap(T &&one, T &&another) {
        T temp = std::forward(one);
        one = std::forward(another);
...
template<typename tkey, typename tvalue>
void search_tree<tkey, tvalue>::merge_nodes(typename search_tree<tkey, tvalue>::common_node *parent, int left_subtree_index)
{
    // TODO: validations
